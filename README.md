...
# Проблемы и будущие доработки
  ...
  - Случайное зависание прибора (может зависнуть при выходе из спящего режима с помощью кнопки) - Возможное решение: переработать/оптимизировать код/. Также устройство может перезапуститься при ручном выходе из сна, из-за бага инициализации датчиков (случается рандомно...), что может уменьшить автономность на некоторое время.
